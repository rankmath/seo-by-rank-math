<?php
/**
 * Unit tests for sitemap router.
 *
 * @since      1.0.0
 * @package    RankMath
 * @subpackage RankMath\Tests\Sitemap
 * @author     MyThemeShop <admin@mythemeshop.com>
 */

namespace RankMath\Tests\Sitemap;
...
class TestRouter extends UnitTestCase {
	...
	public function test_sitemap_urls() {
		...
		$this->assertEquals(
			Router::get_base_url( Sitemap::get_sitemap_index_slug() . '.xml' ),
			'http://example.org/sitemap_index.xml'
		);

		$this->assertEquals(
			Router::get_base_url( 'sitemap.xml' ),
			'http://example.org/sitemap.xml'
		);

		$this->assertEquals(
			Router::get_base_url( 'sitemap-post-2019-01.xml' ),
			'http://example.org/sitemap-post-2019-01.xml'
		);
		...
		$this->assertEquals(
			Router::get_base_url( 'sitemap-post-2019-01.xml' ),
			'https://example.org/sitemap-post-2019-01.xml'
		);
	 ...
	 */
	public function needs_sitemap_index_redirect( $server_vars, $home_url, $wp_query, $expected ) {
		$server_orig   = $_SERVER;
		...
		$result = self::$instance->needs_sitemap_index_redirect();
		$message = \MyThemeShop\Helpers\Url::get_current_url() . ' = ' . $home_url;
	...
	public function data_needs_sitemap_index_redirect() {
		$server_vars_sets = array(
			...
			array(
				'SERVER_NAME' => 'testsite.org',
				'REQUEST_URI' => '/' . Sitemap::get_sitemap_index_slug() . '.xml',
			),
			array(
