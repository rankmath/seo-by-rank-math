			...
			{ showProNotice && (
				<div className="components-notice rank-math-notice is-warning">
					<div className="components-notice__content">
						{ __( 'Multiple Schemas are allowed in the', 'rank-math' ) } <strong>Pro Version [Coming Soon]</strong>
					</div>
				</div>
			) }
...
export default compose(
	withSelect( ( select ) => {
		...
		const count = Object.keys( select( 'rank-math' ).getSchemas() ).length

		return {
			schemas: select( 'rank-math' ).getSchemas(),
