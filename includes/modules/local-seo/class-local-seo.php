...
class Local_Seo {
	...
	public function search_pages() {
		...
		$pages = wp_cache_get( 'rank_math_local_seo_search_pages_' . $term );
		if ( false === $pages ) {
			global $wpdb;
			$pages = $wpdb->get_results( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery -- Direct DB query is required.
				$wpdb->prepare(
			...
			wp_cache_set( 'rank_math_local_seo_search_pages_' . $term, $pages, '', HOUR_IN_SECONDS );
		}
	...
	private function organization( $entity, $data ) {
		...
		// Price Range.
		$price_range = Helper::get_settings( 'titles.price_range' );
		if ( $price_range ) {
		...
		return Sanitizer::sanitize_organization_schema( $entity, $type );
	}
