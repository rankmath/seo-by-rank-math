...
export default () => {
	...
	const runCommand = () => {
		...
		insertCommandBox( 'Write', getWriteAttributes( getBlockContent( selectedBlock ) ), selectedBlock.clientId )
	}
	...
	document.addEventListener( 'keyup', ( event ) => {
		...
		let content = getBlockContent( selectedBlock )
		...
		if ( startsWith( content, '//' ) && selectedBlock.name === 'core/paragraph' ) {
			const newBlock = createBlock( 'rank-math/command', {
				content: getBlockContent( selectedBlock ).replace( '//', '<span>//</span>' ),
