...
const useBlock = () => {
	...
	const content = selectedBlock.attributes.content.replace( /<div .*<\/div>/g, '' ).replaceAll( '  ', '' )
...
const writeMore = () => {
	const selectedBlock = select( 'core/block-editor' ).getSelectedBlock()
	const clientId = selectedBlock.clientId
	const content = selectedBlock.attributes.content.replace( /<div .*<\/div>/g, '' ).replace( '<br>', '' ).replaceAll( '  ', '' )
	updateBlockAttributes(
		clientId,
		{
