...
class LLMS_Txt {
	...
	private function add_header_content() {
		...
		$site_title = $this->format_text( Helper::get_settings( 'titles.knowledgegraph_name', get_bloginfo( 'name' ) ) );
		$summary    = $this->format_text( Helper::replace_vars( Helper::get_settings( 'general.llms_summary', '' ) ) );

		$this->output_line( '# ' . $site_title );
		$this->output_line( '' );

		if ( $summary ) {
			$this->output_line( '> ' . $summary );
			$this->output_line( '' );
		}
		...
		$this->output_line( '- [XML Sitemap](' . esc_url_raw( $sitemap_url ) . '): Includes all crawlable and indexable pages.' );
		$this->output_line( '' );
	}
	...
	private function add_post_types_data( $limit ) {
		...
		foreach ( $post_types as $post_type ) {
			...
			$label = $this->format_text( get_post_type_object( $post_type )->labels->name );
			$this->output_line( '## ' . $label );
			...
			foreach ( $posts as $object ) {
				...
				$title                = $this->format_text( get_the_title( $object ) );
				$link                 = esc_url_raw( get_permalink( $object ) );
				 ...
				 */
				$desc = $this->format_text( $this->do_filter( 'llms_txt/post_description', $desc, $object ) );
	...
	private function add_taxonomies_data( $limit ) {
		...
		foreach ( $taxonomies as $taxonomy ) {
			...
			$label = $this->format_text( $tax_obj->labels->name );
			$this->output_line( '## ' . $label );
			foreach ( $terms as $term ) {
				if ( $term && Helper::is_term_indexable( $term ) ) {
					$name = $this->format_text( $term->name );
					$link = esc_url_raw( get_term_link( $term ) );
					$desc = $this->format_text( $term->description );
	...
	private function add_extra_content() {
		...
		if ( ! empty( $extra ) ) {
			$this->output_line( $this->format_text( $extra ) );
		}
	}

	/**
	 * Normalizes text for plain-text output: strips tags, decodes entities.
	 * Decode first, so an entity-encoded tag (e.g. &lt;script&gt;) can't survive stripping.
	 *
	...
	/**
	 * Outputs a line with a trailing newline.
	 *
	 * @param string $content Plain text content; must already be sanitized (see format_text()).
	 * @return void
	 */
	private function output_line( $content ) {
		echo $content . "\n"; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Plain text, not HTML; see format_text().
	}
}
