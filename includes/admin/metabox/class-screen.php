...
class Screen implements IScreen {
	...
	private function power_words() {
		static $words;
		$locale = Locale::get_site_language();
		$locale = sanitize_key( $locale ); // Just for good measure.
		$file   = rank_math()->plugin_dir() . 'assets/vendor/powerwords/' . $locale . '.php';
		if ( ! file_exists( $file ) ) {
			return false;
		}
		$words = $words ? $words : include $file; // phpcs:ignore WordPressVIPMinimum.Files.IncludingFile.UsingVariable - The include is safe.
	...
	private function diacritics() {
		...
		$diacritics = include_once $file; // phpcs:ignore WordPressVIPMinimum.Files.IncludingFile.UsingVariable - The include is safe.
