...
<?php endif; ?>
			...
			<th>
				...
				if ( $php_version_ok ) {
					...
					if ( $php_version_recommend ) {
						...
						<p class="description">
						<?php
							echo ( ! Helper::is_whitelabel() ) ? // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- This is a false positive.
								esc_html__( 'Rank Math is compatible with your PHP version but we recommend updating to PHP 7.2 for increased speed and security.', 'rank-math' ) . ' <a href="' . esc_url( KB::get( 'rm-requirements' ) ) . '" target="_blank">' . esc_html__( 'More information', 'rank-math' ) . '</a>' : // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- This is a false positive.
	...
	<?php if ( $all_good ) { ?>
		...
		<?php
			echo ( ! Helper::is_whitelabel() ) ? // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- This is a false positive.
				esc_html__( 'Your server is correctly configured to use Rank Math.', 'rank-math' ) : // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- This is a false positive.
	...
	<?php } else { ?>
		...
		<?php
			echo ( ! Helper::is_whitelabel() ) ? // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- This is a false positive.
				esc_html__( 'Please resolve the issues above to be able to use all features of Rank Math plugin. If you are not sure how to do it, please contact your hosting provider.', 'rank-math' ) : // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- This is a false positive.
	...
	<?php if ( $conflicting_plugins ) : ?>
		<p class="conflict-text">
			<?php
				echo ( ! Helper::is_whitelabel() ) ? // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- This is a false positive.
					esc_html__( 'The following active plugins on your site may cause conflict issues when used alongside Rank Math: ', 'rank-math' ) : // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- This is a false positive.
